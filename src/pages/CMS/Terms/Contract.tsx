...
const Contract: React.FC = () => {
  ...
  const loadDocument = useCallback(async () => {
    try {
      setIsLoading(true);
      const activeDoc = await dbManager.terms.getActiveTermsDocument('contract');
      if (activeDoc) {
        setCurrentDocument(activeDoc);
        setContent(activeDoc.content);
        devLog("계약서 문서 로딩:", activeDoc);
      } else {
        setCurrentDocument(null);
        setContent("");
        devLog("새 계약서 문서 생성 준비");
      }
    } catch (err: any) {
      devLog("계약서 문서 로딩 실패:", err);
    ...
    try {
      ...
      if (currentDocument) {
        ...
        if (updatedDoc) {
          ...
          toast.success("계약서가 성공적으로 수정되었습니다!");
        }
      } else {
        // 새 문서 생성
        ...
        toast.success("계약서가 성공적으로 저장되었습니다!");
      }

      await loadDocument();
    } catch (err: any) {
      devLog("계약서 저장 실패:", err);
      ...
      <EditorContainer>
        {isLoading ? (
          ...
          <CustomTiptapEditor
            ...
            key={`contract-${currentDocument?.id || 'new'}`}
